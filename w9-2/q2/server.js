...
app.use(express.static(path.join(__dirname, '9-2/build')));
...
const mainPage = (req, res) => {
    ...
    res.sendFile(path.join(__dirname, '9-2/build/index.html'));
}

const listUsers = (req, res) => {
    console.log("회원명단 조회요청을 받았으며, 리액트에게 명단을 보냅니다");
    res.json(userList); // .json는 객체/배열을 클라이언트에게 그대로 보낼
}
...
app.post("/users/check", CheckUsers); // REST API 바인딩 (회원확인)
